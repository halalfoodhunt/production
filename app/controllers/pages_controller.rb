...
def friends_rewards
  ...
  if params[:friends_reward].blank?
    ...
    @food_deliveries = FoodDeliveries.where(friends_reward_id: @friends_reward_id).order("created_at DESC")
  end
end
...
def places_friends_rewards
  ...
  if params[:friends_reward].blank?
    @search = Place.ransack(params[:q])
    @places = @search.result.where(draft: false).where.not(friends_rewards_terms: nil)
  else
    @friends_reward_id = FriendsReward.find_by(name: params[:friends_reward]).id
